...
export default function DocTimeline({ owner, section, isMyPage, baseDate, viewportH, profiles, mentionFreq, mentionStates, flashKey }) {
  ...
  const carry = useCallback(
    (fromPeriod, blockIndex) => {
      moveBlockToToday({ owner, fromSection: section, fromPeriod, blockIndex, toSection: section, toPeriod: curKey })
        .then(() => refresh())
        .catch((e) => {
  ...
  if (section === 'stash') {
    return (
      <div id="doc-stash-stash" className={'pt-1' + (flashKey === 'stash' ? ' doc-flash' : '')}>
        <DocBlock key={`stash-${owner}`} owner={owner} section="stash" periodKey="stash" editable={isMyPage} placeholder="写点什么…" profiles={profiles} mentionFreq={mentionFreq} mentionStates={mentionStates} fill />
      </div>
    )
  }
  ...
  return (
    <>
      ...
      <div id={`doc-${section}-${curKey}`} className={'mb-3' + (flashKey === curKey ? ' doc-flash' : '')}>
        ...
        <DocBlock key={`${owner}-${section}-${curKey}-${reloadNonce}`} owner={owner} section={section} periodKey={curKey} editable={isMyPage} placeholder="写点什么…" profiles={profiles} mentionFreq={mentionFreq} mentionStates={mentionStates} fill />
      </div>
      {/* 过去：只读，往下回溯 */}
      {pastKeys.map((k) => (
        <div key={`${owner}-${section}-${k}-${reloadNonce}`} id={`doc-${section}-${k}`} className={'mb-3' + (flashKey === k ? ' doc-flash' : '')}>
