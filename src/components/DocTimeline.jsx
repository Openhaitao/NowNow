...
export default function DocTimeline({ owner, section, isMyPage, baseDate, viewportH }) {
  ...
  if (section === 'stash') {
    return (
      <div className="pt-1" style={viewportH ? { minHeight: viewportH } : undefined}>
        <DocBlock owner={owner} section="stash" periodKey="stash" editable={isMyPage} placeholder="写点什么…" />
      </div>
    )
  }
  ...
  return (
    <>
      ...
      <div className="mb-3" style={viewportH ? { minHeight: viewportH } : undefined}>
        ...
        <DocBlock owner={owner} section={section} periodKey={curKey} editable={isMyPage} placeholder="写点什么…" />
      </div>
      {/* 过去：只读，往下回溯 */}
      {pastKeys.map((k) => (
        <div key={k} className="mb-3">
          <div className="pb-0.5 pt-3 text-[13px] font-medium text-stone-500">{periodHeaderFromKey(section, k)}</div>
          <DocBlock owner={owner} section={section} periodKey={k} editable={false} />
        </div>
      ))}
    </>
  )
