...
function Avatar({ p, size = 'h-[22px] w-[22px] text-[11px]', highlight }) {
  return (
    <span
      className={
        `flex shrink-0 items-center justify-center rounded-full ${size} ` +
        (highlight ? 'bg-blue-600 text-white' : 'bg-emerald-100 text-emerald-900')
      }
    >
      {p.display_name[0]}
    </span>
    ...
    <div className="flex min-h-screen">
      ...
      <aside className="hidden w-44 shrink-0 flex-col border-r border-stone-100 bg-stone-50/60 px-2.5 py-4 md:flex">
        <div className="mb-3 flex items-center gap-1.5 px-2 text-[13px] font-semibold">
          <img src="/logo.png" alt="" className="h-5 w-5 rounded" /> NowNow
        </div>
        {profiles.map((p) => (
          <button
            key={p.id}
            ...
            className={
              'flex items-center gap-2 rounded-lg px-2 py-1.5 text-left text-[13px] ' +
              (p.id === pageUserId ? 'bg-blue-50 font-medium text-blue-700' : 'text-stone-600 hover:bg-stone-100')
            }
          >
            <Avatar p={p} highlight={p.id === pageUserId} />
        ...
        <div className="mt-3 px-2 text-[11px] leading-relaxed text-stone-400">点人名进入对方的纸（只读）</div>
        <button
          onClick={() => supabase.auth.signOut()}
          className="mt-auto px-2 py-1 text-left text-xs text-stone-400 hover:text-stone-600"
        >
          退出登录
        </button>
      </aside>
      ...
      <main className="min-w-0 flex-1">
        {/* 移动端：顶部头像横排 */}
        <div className="flex items-center gap-2 border-b border-stone-100 px-4 py-2.5 md:hidden">
          <img src="/logo.png" alt="" className="h-6 w-6 rounded" />
          <div className="flex flex-1 gap-1.5 overflow-x-auto">
            {profiles.map((p) => (
              <button key={p.id} onClick={() => viewPage(p.id)} className="relative shrink-0">
                <Avatar p={p} size="h-7 w-7 text-xs" highlight={p.id === pageUserId} />
        ...
        <div className="mx-auto max-w-2xl px-5 pb-24 pt-2 md:px-8">
          {!isMyPage && (
